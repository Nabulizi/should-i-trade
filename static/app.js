...
function buildRadarChart(pillars) {
  ...
  svg += `<polygon points="${pts.map(p=>p.map(v=>v.toFixed(1)).join(',')).join(' ')}" fill="rgba(59,130,246,0.15)" stroke="var(--accent)" stroke-width="1.5"/>`;
  // Dots + labels
  keys.forEach((k,i) => {
...
const SPARK_COLORS = { total: '#ededee', v: '#3b82f6', tr: '#00e676', br: '#ffd740', mo: '#ff9100', ma: '#8a8d93' };
let _sparkHistory = [];
